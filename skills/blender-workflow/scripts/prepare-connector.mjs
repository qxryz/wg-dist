...
import { repairWindowsDependencies } from './windows-dependencies.mjs';

// Adapter for the desktop connector-preparation contract. The desktop owns
// download, elevation, registration and hot-apply. Windows recovery adds only
// the pinned missing pywin32 dependency inside the staged Blender package.
...
If a command runner needs a timeout, allow at least 1500000 ms for --install.
On Windows addon failure, inspect the managed Python bundle, repair a proven
missing pywin32 dependency from a pinned, hash-verified wheel, then retry once.
Repair stays inside the registered package; global Python and preferences are unchanged.
...
Installed/MCP-connected does not prove Blender is running or its addon responds.
Read the actual scene through the current conversation before editing.
This helper prepares the community Blender connector; it does not switch providers.`;
...
function parseResult(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
      || value.connectorId !== 'blender' || typeof value.ok !== 'boolean'
  ...
  const result = { ok: value.ok, connectorId: 'blender', state: value.state,
...
export async function prepareConnector({ args = [], env = process.env, fetchImpl = fetch,
  timeoutMs = 600000, platform = process.platform, repairImpl = repairWindowsDependencies } = {}) {
  ...
  async function request(action) {
    try {
      const response = await fetchImpl(target.url, {
        ...
        body: JSON.stringify({ connectorId: 'blender', action }),
      ...
      return failure('preparation_result_unavailable',
        'No verified result. Installation may still be running: check --status before retrying. No Blender control has been verified.');
    }
  }
  ...
  if (platform === 'win32' && !installed.ok && installed.state === 'failed'
      && installed.code === 'addon_install_failed') {
    ...
    if (!current.packageDir) return { ...installed, recovery: { ok: false, code: 'package_path_unavailable' } };
    const recovery = await repairImpl({ packageDir: current.packageDir, env });
    if (!recovery.ok || !recovery.repaired) return { ...installed, recovery };
    const checked = await request('status');
    if (!checked.ok || checked.state === 'installing'
        || (checked.state === 'installed' && checked.mcpConnected)) return { ...checked, recovery };
    if (checked.packageDir !== current.packageDir) return {
      ...failure('package_changed', 'The staged package changed during repair. Check current status before another installation.'), recovery,
    };
    return { ...await request('install'), preparationRetried: true, recovery };
  }
  if (installed.ok || installed.state !== 'failed'
      || installed.code !== 'package_download_failed') return installed;
  // A terminal package failure can be retried via the client's managed flow,
  // which owns cooldown bypass. Recheck first: another session may be working.
  const current = await request('status');
  if (!current.ok || current.state === 'installing'
      || (current.state === 'installed' && current.mcpConnected)) return current;
