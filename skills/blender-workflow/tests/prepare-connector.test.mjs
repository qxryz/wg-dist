...
const status = (state, connected = false) => ({ ok: true, connectorId: 'blender',
...
test('status only inspects and carries current workspace headers', async () => {
  ...
  const result = await prepareConnector({ env: context, fetchImpl: async (url, init) => {
    ...
    assert.deepEqual(JSON.parse(init.body), { connectorId: 'blender', action: 'status' });
...
test('busy state, forged Blender verification and invalid arguments do not install', async () => {
