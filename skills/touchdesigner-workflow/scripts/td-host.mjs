...
const config = {"vendor":"Derivative","exe":"TouchDesigner.exe","macName":"touchdesigner","files":[".toe",".tox"]};
...
async function launchApp(platform, app, file) {
  if (platform === 'darwin') {
    await execute('/usr/bin/open', ['-a', app, ...(file ? [file] : [])], { timeout: 15000 });
  } else {
    // The application window is intentional. No shell or visible console helper.
    await new Promise((resolve, reject) => {
      const child = spawn(app, file ? [file] : [], { shell: false, detached: true, stdio: 'ignore' });
