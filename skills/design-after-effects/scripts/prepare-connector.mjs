...
Installed/MCP-connected does not prove AE is running or its bridge responds.
Use packageDir with ae-host.mjs inspect even if startupSupported is false:
that flag describes a package-declared launcher, not skill compatibility adapters.`;
...
function parseResult(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
      || value.connectorId !== 'after-effects' || typeof value.ok !== 'boolean'
  ...
  const result = { ok: value.ok, connectorId: 'after-effects', state: value.state,
  ...
  async function request(action) {
    try {
      const response = await fetchImpl(target.url, {
        ...
        body: JSON.stringify({ connectorId: 'after-effects', action }),
      ...
      return failure('preparation_result_unavailable',
        'No verified result. Installation may still be running: check --status before retrying. No AE control has been verified.');
    }
  }
