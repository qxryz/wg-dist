...
Installed/MCP-connected does not prove Photoshop is running or document control responds.
Read the actual document list through the current conversation before editing.
This helper prepares the registered Photoshop connector; it does not load application plugins.`;
...
function parseResult(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
      || value.connectorId !== 'photoshop' || typeof value.ok !== 'boolean'
  ...
  const result = { ok: value.ok, connectorId: 'photoshop', state: value.state,
  ...
  async function request(action) {
    try {
      const response = await fetchImpl(target.url, {
        ...
        body: JSON.stringify({ connectorId: 'photoshop', action }),
      ...
      return failure('preparation_result_unavailable',
        'No verified result. Installation may still be running: check --status before retrying. No Photoshop control has been verified.');
    }
  }
