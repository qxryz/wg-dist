...
description: |
  制作粗糙发光手绘动画与实拍空间融合的十五秒十六比九短片。先确认接触物体或手部动作、情绪、语言和风格，再设计连续变形、逃跑路线与慢半拍手持追拍；用户批准提示词后默认用 H3 生成，并检查触碰真实感、镜头延迟、粗糙笔触和非恐怖调性。适合单场景创意短片，不用于精致三维动画、恐怖跳吓、毛绒角色或多场景剪辑。
trigger-words: [手绘发光动画实拍融合, 15秒变形追逐视频提示词, H3生成视频, 视频prompt, 手绘动画接触真实物体, 蜡笔粉笔质感, 多语言视频提示词]
...
- 用户明确确认使用默认 MiniMax-H3 或明确指定的其他模型前，不生成视频、图片、音频、分镜或中间资产。
- 默认模型是 MiniMax-H3。如果用户明确指定其他模型，经过能力检查后遵循用户选择。除非用户要求，否则不要在创意 prompt 内添加模型参数。用户指定的模型生成失败时，最多针对性重试一次，仍失败则切换 MiniMax-H3 或其他可用模型，不要反复重试同一模型。
