# Typography Product TVC · 产品贴字广告导演
...
1. **给一张参考图 + 一句话需求**：例如“帮我做一条银色轿跑的 15 秒 TVC”，AI 自动进入完整创意流：需求拆解 → 创意构思 → 画风确认 → 资产生成 → 分镜 + 视频脚本。
2. **产品多视图提示词 → 图像生成模型 出图**：AI 输出产品多视图提示词，用户复制到 图像生成模型 edit 模式并传入参考图，得到标准化多角度产品图。
3. **9 宫格分镜提示词 → 图像生成模型 出图**：AI 输出 3×3 多宫格分镜提示词，包含逐格构图、光影、运镜和文字动效位置，用户传入多视图 + 环境图得到完整分镜。
4. **视频提示词 → MiniMax H3 分段视频 → 确认 → 合成成片**：AI 按分镜计划拆成对应片段，分别生成 MiniMax H3 视频；用户确认片段 OK 后，再按顺序合成为最终视频，并根据产品品类匹配专属背景音。
...
本 Skill 现在按固定顺序执行：1）上传参考图和品牌名；2）生成产品多视角图并确认；3）生成 3×3 分镜图并确认；4）按分镜生成 3–5 段视频并逐段确认；5）合成所有确认片段并添加匹配产品品类的 BGM。若品牌名、参考图用途、主产品或产品外观不清晰，必须先询问，不得直接生成。

## 主参考：方形拼贴

Skill 现在以 `live-action-typography.mp4` 的运镜与文字动画作为主要参考：文字要像真实空间中的功能标签，围绕产品、人物、手部、脚下、产品零件和运动轨迹产生环绕、跟踪、遮挡与转场。分镜图和分段视频会默认使用这套参考来约束运镜与文字动效，并要求动画自然丝滑：文字缓入缓出、带适度动态模糊、排版克制清晰、与人物/产品动作同步，不允许出现卡顿、文字混乱、文字遮挡产品、人物与文字配合生硬等情况。

## 三类文字动画参考智能匹配

Skill 现在把 `reference-674fc8d5.mp4` 作为主参考：它强调方形双线拼贴、拼贴式开场、双线叙事、局部微距与宏观动作交替、文字作为结构件、以及动作与文字同步的平滑转场。A「K-pop 空棚群像贴字」适合年轻群像、潮流消费、运动服饰和轻盈社交场景；B「实拍商业 3D 空间功能词」适合耳机、手机、智能硬件、运动性能和人机互动；C「环形折叠动态字体」适合几何产品、包装、Logo、视觉钩子和高级 motion design。生成 3×3 分镜图和每个 MiniMax H3 分段视频时，Skill 会根据产品特点、卖点和风格智能选择主参考与辅助参考，并在片段提示词中写明选择原因。

## 耳机广告模板

Skill 已把 `reference-674fc8d5.mp4` 的画面下方耳机广告整理为标准模板：画面上方保留人物或信息线，画面下方负责耳机产品线，文字必须是 3D 空间功能词、圆环字、扫屏大字或参数标签，并与人物动作、产品旋转和音乐重拍同拍。

## 分段确认与背景音

9 宫格分镜确认后，Skill 不会直接一次性生成最终成片，而是先按分镜计划生成 3–5 个 MiniMax H3 片段。每个片段单独确认，全部 OK 后再合成为完整 TVC。主参考默认为方形拼贴与双线叙事，背景音会根据产品品类选择：科技数码偏电子低频，运动户外偏轻快运动节奏，美妆香氛偏精致 editorial beat，食品饮料偏明快 pop，汽车机械偏 cinematic hybrid。
...
- **即拷即用** — 产出的提示词可直接粘贴到 图像生成模型 / MiniMax H3，无需二次加工
...
### Step 2 — AI 生成产品多视图提示词 → 图像生成模型 出图

AI 输出产品多视图提示词，复制到 图像生成模型（edit 模式，传入参考图），得到标准化的多角度产品图：

<img src="https://github.com/user-attachments/assets/9d97df38-79f1-4a6a-a6bb-0fdbd731faca" width="600" alt="产品多视图" />

### Step 3 — AI 生成 9 宫格分镜提示词 → 图像生成模型 出图

AI 输出 3×3 多宫格分镜提示词（包含逐格构图、光影、运镜描述），复制到 图像生成模型（edit 模式，传入多视图 + 环境图），得到完整分镜：

<img src="https://github.com/user-attachments/assets/1ca54c8a-e1c2-4a13-8e67-461ea327f2ab" width="600" alt="9 宫格分镜" />

### Step 4 — AI 生成视频提示词 → MiniMax H3 生成视频

AI 同步输出 MiniMax H3 视频提示词（5 Phase / 15s），配合多宫格作为首帧 + 产品多视图作为锚定，双图输入 MiniMax H3 生成最终视频。

### 产出物一览

| 产出物 | 工具 | 用途 |
|--------|------|------|
| 产品多视图 | 图像生成模型 (edit) | 产品锚定，供后续步骤引用 |
| 9 宫格分镜图 | 图像生成模型 (edit) | 视频首帧 + 视觉校对 |
| Multi-Phase 视频脚本 | MiniMax H3 | 生成 15s 成片 |
| 创意方案文档 | — | 完整创意 brief 存档 |

## 设计理念

传统 AI 生成广告往往只是"产品 + 黑底 + 旋转"的无限循环。Typography Product TVC 不同——它按真实 TVC 制片 SOP 组织提示词生成，让每条提示词都有明确的制作环节归属：
...
│       └── ...
│
└── video-scripts/                  # Multi-Phase 视频提示词（MiniMax H3）
...
1. **产品多视图** — 将 `assets/prompts/product-multiview.md` 中的提示词复制到 图像生成模型，选择 edit 模式，传入参考图
2. **分镜关键帧** — 将 `keyframes/prompts/` 下的提示词复制到 图像生成模型，edit 模式，传入多视图 + 环境图
3. **视频脚本** — 将 `video-scripts/` 下的 Multi-Phase 脚本配合多宫格首帧 + 产品多视图，输入 MiniMax H3 生成视频
